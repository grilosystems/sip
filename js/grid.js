	...
	*/
	window.Grid = Root.jQueryPlugin("grid",{
		...
		_construct : function() {
			...
			var $wrapper = $("<div class='gridWrapper'><span class='gridLoading'>Cargando</span></div>");
		...
		*******/
		...
		_afterLoad : function() {
			...
			if(this.opts.deleting) {
				...
				// add the column with a width
				var $deleteCol = this.addColumn("Eliminar", {width: 65, cellClass : "center"}, function() {
		...
		_templates : {
			...
			confirm : ""+
				"<div class='dialog gridConfirm'>\
					<span>{{msg}}</span>\
					<div class='buttons'>\
						<button class='btn confirmOk'>Si</button>\
			...
			alert : ""+
				"<div class='dialog gridAlert {{type}}'>\
					...
					<div class='buttons'>\
						<button class='btn cancel'>Aceptar</button>\
					</div>\
				</div>\
			",
			notify : ""+
			...
			pager : ""+
				"<div class='pagination left'>\
				  <ul>\
				  	<li class='disabled'>\
				  		<a href='#' class='pager_showing'>p&aacute;gina \
				  			<span class='pager_lower_limit'>{{start}}</span> - \
				  			<span class='pager_upper_limit'>{{end}}</span>\
				  		</a></li>\
				    <li class='gridPrev'><a href='#'>Anterior</a></li>\
				    <li class='gridNext'><a href='#'>Siguiente</a></li>\
		...
		load : function(opts) {
			...
			// show loading box
			this.loadingDialog = this.notify("Cargando");
		...
		*/
		...
		deleteRow : function(e,el) {
			...
			this.confirm("&iquest;Quiere eliminar este registro?", function() {
		...
		saveRow : function(e,el) {
			...
			var pkeys = []; //Almacena los ID de las filas modificadas
			...
			$.post(this.opts.action,{ 
				...
				if(res == 1) {
					self.alert("info","Modificada!",i + " Fila(s) Modificada");
					$(self.el).trigger("save",rows[pkey],res);
					filasModificadas(pkeys);
				} else {
					self.error(res);
